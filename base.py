...
LINKS = {
    "github": "https://github.com/vairous-1x/EduScrape/",
    "support": "https://www.vairous1x.com/2024/03/eduscrape.html",
    "telegram": "https://t.me/+jZwbmEYZpEhlNGI0",
}
...
class Udemy:
    ...
    def load_settings(self):
        try:
            with open(f"EduScrape-{self.interface}-settings.json") as f:
                self.settings = json.load(f)
        except FileNotFoundError:
            self.settings = dict(
                requests.get(
                    f"https://raw.githubusercontent.com/vairous-1x/EduScrape/master/EduScrape-{self.interface}-settings.json"
                ).json()
            )
    ...
    def save_settings(self):
        with open(f"EduScrape-{self.interface}-settings.json", "w") as f:
    ...
    def check_for_update(self) -> tuple[str, str]:
        r_version = (
            requests.get(
                "https://api.github.com/repos/vairous-1x/EduScrape/releases/latest"
            )
            .json()["tag_name"]
            .removeprefix("v")
        )
        ...
        if c_version < r_version:
            ...
            return f"Login {c_version}", f"EduScrape {c_version}"
        else:
            return (
                f"Dev Login {c_version}",
                f"EduScrape {c_version}",
            )
